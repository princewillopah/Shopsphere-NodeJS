...
pkill -f "port-forward.*shopsphere" || true

# Force delete the namespace and explicitly wait for it to be fully gone
if kubectl get namespace "${NAMESPACE}" >/dev/null 2>&1; then
    echo "♻️ Removing old namespace..."
...
# Create a fresh namespace safely
echo "✨ Creating namespace: ${NAMESPACE}"
kubectl create namespace "${NAMESPACE}"

# =================================================================
# RESOURCE APPLICATION (Updated with target namespace flags)
# =================================================================

# 4. Create configs and secrets (Added explicit namespace target)
kubectl apply -f secrets/backend-sealed-secret.yaml -n ${NAMESPACE}
kubectl apply -f secrets/aws-s3-sealed-secret.yaml -n ${NAMESPACE}
kubectl apply -f configmaps/backend.yaml -n ${NAMESPACE}
kubectl apply -f configmaps/frontend.yaml -n ${NAMESPACE}

# 5. DB must be running
kubectl apply -f database/pvc.yaml -n ${NAMESPACE}
kubectl apply -f database/deployment.yaml -n ${NAMESPACE}
kubectl apply -f database/service.yaml -n ${NAMESPACE}
kubectl wait --for=condition=ready pod -l app=db -n ${NAMESPACE} --timeout=120s

# 6. Run MIGRATION job
kubectl apply -f jobs/migrate-job.yaml -n ${NAMESPACE}
...
# 8. Apply the Horizontal Pod Autoscaler and Pod Disruption Budget
kubectl apply -f autoscaling/hpa.yaml -n ${NAMESPACE}
kubectl apply -f autoscaling/pdb.yaml -n ${NAMESPACE}

# 9. Run SEED job
kubectl apply -f jobs/seed-job.yaml -n ${NAMESPACE}
kubectl wait --for=condition=complete job/backend-seed -n ${NAMESPACE} --timeout=120s
kubectl logs job/backend-seed -n ${NAMESPACE} 

# 10. Deploy frontend
kubectl apply -f frontend/deployment.yaml -n ${NAMESPACE}
kubectl apply -f frontend/service.yaml -n ${NAMESPACE}

# 11. Network Policies & Ingress
kubectl apply -f network-policy/backend-allow-frontend.yaml -n ${NAMESPACE}
kubectl apply -f network-policy/db-allow-only-backend.yaml -n ${NAMESPACE}
